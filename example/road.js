...
this.router = function(app) {
    for(var index in config.routes) {
        ...
        if(route[0] == 'map') {
            // custom routes
            var method = route[1], path = route[2], controllerName = route[3], actionName = route[4];
            app[method].apply(null, [path, mappedResponse(controllerName, actionName)]);
        } else {
            // default routes
		    var method = route[0], path = route[1];
		    app[method].apply(null, [path,
                function(req,res) {
...
function callController(controllerName, functionName, req, res) {
    ...
    var connection = {'req': req, 'res': res};
    controller[functionName].apply(null,
        [connection, createControllerCallback(controllerName, req, res)]);
}
